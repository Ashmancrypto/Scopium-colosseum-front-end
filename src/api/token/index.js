...
export async function findTokens(
  ...
  const result = await axiosPublic.get(
    `/token/find_tokens?name=${name}&sort_condition=${sort_condition.replace(
      "sort: ",
      ""
    )}&sort_order=${sort_order.replace(
      "sort: ",
      ""
    )}&include_nsfw=${nsfw}&userId=${userId}`
  );
  return result.data;
}
...
export async function getTrendingTokens(userId) {
  const result = await axiosPublic.get(
    `/token/get_trending_tokens?userId=${userId}`
  );
  return result.data;
}
...
export async function getToken(mintAddr, userId) {
  ...
  const result = await axiosPublic.get(
    `/token/get_token_info?mintAddr=${mintAddr}&userId=${userIdStr}`
  );
  return result.data;
}
...
export async function getThreadData(mintAddr, userId) {
  const userIdStr = encodeURIComponent(JSON.stringify(userId));
  const result = await axiosPublic.get(
    `/token/get_thread_data?mintAddr=${mintAddr}&userId=${userIdStr}`
  );
  return result.data;
}
...
export async function likeReply(replyMentionId) {
  const result = await axiosPrivate.post("/token/reply_like", {
    replyMentionId,
  });
  return result;
}
...
export async function dislikeReply(replyMentionId) {
  const result = await axiosPrivate.post("/token/reply_dislike", {
    replyMentionId,
  });
  return result;
}
...
export async function getTradeHistory(mintAddr) {
  const result = await axiosPublic.get(
    `/token/get_trade_hist?mintAddr=${mintAddr}`
  );
  return result.data;
}
...
export async function getMarketId(baseMint, quoteMint) {
  const result = await axiosPublic.get(
    `/token/get_marketid?baseMint=${baseMint}&quoteMint=${quoteMint}`
  );
  return result.data;
}
...
export async function getMintPair(category) {
  ...
  if (mintBytes.length !== 64) {
    throw new Error(
      `Decoded secret key has invalid length: ${mintBytes.length}`
    );
  }
