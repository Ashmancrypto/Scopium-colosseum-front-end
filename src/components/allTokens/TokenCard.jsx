...
const TokenCard = ({ token, maxWidth = false }) => {
  ...
  const handleWatchlistToggle = async (e) => {
    ...
    if (!currentUser) {
      toast.warning(
        "Login Required",
        "Please log in to add tokens to favorites."
      );
      return;
    }
  ...
  return (
    <div
      ...
      className={`relative rounded-[12px] px-[16px] sm:pb-[35px] pb-[20px] pt-12 shadow-black/20 shadow-sm border border-[rgba(10,10,10,0.5)] text-center hover:scale-[1.01] transition-all cursor-pointer mb-8 sm:h-[78px] w-full ${
        isDark
    ...
    >
      ...
      <button
        ...
        className={`hidden sm:absolute top-2 right-2 hover:scale-110 transition-transform ${
          isWatchlisted
            ? isDark
              ? "text-green-400"
              : "text-pink-400"
      ...
      {/* Token Info */}
      <div
        className={`mt-1 sm:flex hidden items-center justify-center gap-[17px] text-sm font-medium transition-colors duration-300 ${
          isDark ? "text-white" : "text-black"
        }`}
      >
        <span className="truncate max-w-[33%] text-left">
          ${token.ticker || "???"}
        </span>
        <span
          className={`truncate max-w-[34%] text-center ${
      ...
      {/* Token Info Mobile */}
      <div
        className={`mt-1 sm:hidden flex flex-col items-center justify-start gap-[10px] text-sm font-medium transition-colors duration-300 ${
          isDark ? "text-white" : "text-black"
        }`}
      >
        <div className="flex items-center justify-between w-full">
          <h4 className="flex-shrink-0 text-left text-[16px] font-bold">
            ${token.ticker || "???"}
          </h4>
          <button
            onClick={handleWatchlistToggle}
            disabled={isUpdatingFavorite}
            className={`sm:hidden top-2 right-2 hover:scale-110 transition-transform ${
              isWatchlisted
                ? isDark
                  ? "text-green-400"
                  : "text-pink-400"
        ...
        <div className="flex items-center justify-between w-full text-[16px]">
          ...
          >
            {formatTokenMarketCap(token.marketCap, 1)}
          </span>
          {(token.priceChange || token.priceChange === 0) && (
            <span
              className={`flex items-center justify-end ${getChangeColor(
