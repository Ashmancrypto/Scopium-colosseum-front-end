...
const FilteredTokensList = ({ tokens, activeFilters, loading, error, onRefresh }) => {
  ...
  const getFilteredTokens = () => {
    ...
    if (activeFilters.trending.length > 0) {
      activeFilters.trending.forEach(filter => {
        switch (filter) {
          case 'newly_created':
            filtered = filtered.sort((a, b) => new Date(b.cdate) - new Date(a.cdate));
            break;
          case 'popular':
            filtered = filtered.sort((a, b) => (b.replies || 0) - (a.replies || 0));
            break;
          case 'biggest_gainers':
            filtered = filtered.filter(token => parseFloat(token.priceChange || 0) > 0)
    ...
    if (activeFilters.marketCap.length > 0) {
      activeFilters.marketCap.forEach(filter => {
        switch (filter) {
          case 'high_to_low':
            filtered = filtered.sort((a, b) => parseFloat(b.marketCap || 0) - parseFloat(a.marketCap || 0));
            break;
          case 'low_to_high':
            filtered = filtered.sort((a, b) => parseFloat(a.marketCap || 0) - parseFloat(b.marketCap || 0));
            break;
        }
      });
    }
    ...
    if (activeFilters.categories.length > 0) {
      filtered = filtered.filter(token => {
        ...
        return activeFilters.categories.some(category => 
          token.category === categoryMap[category]
        );
      });
    }
