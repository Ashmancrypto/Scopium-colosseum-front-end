...
const HamburgerMenu = ({ onMenuToggle }) => {
  ...
  const legal = [
    ...
    { name: "Disclaimer", url: "/disclaimer", isRoute: true },
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <div className="w-6 h-6 flex flex-col justify-center space-y-1">
          <div
            className={`h-0.5 ${
              isDark ? "bg-green-600" : "bg-pink-600"
      ...
      {isMenuOpen && (
        <div
          ...
          } rounded-lg shadow-lg border border-green-200 py-2 z-50`}
        >
          <span className="flex items-center px-7 font-bold">General</span>
          {general.map((item, index) => (
            <button
              key={index}
              className={`flex items-center space-x-3 px-4 py-1 text-gray-700 hover:bg-gray-50 ${
                isDark ? "hover:text-green-600" : "hover:text-pink-600"
          ...
          {legal.map((item, index) => (
            <button
              key={index}
              className={`flex items-center space-x-3 px-4 py-1 text-gray-700 ${
                isDark ? "hover:text-green-600" : "hover:text-pink-600"
              } hover:bg-gray-50 transition-colors duration-200 w-full text-left`}
              onClick={() => handleLinkClick(item)}
            >
              <span className="text-lg">{item.icon}</span>
              <span className="font-medium">{item.name}</span>
