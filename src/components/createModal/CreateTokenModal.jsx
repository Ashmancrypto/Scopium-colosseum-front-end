...
const CreateTokenModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.coinName || !formData.ticker || !formData.category) {
      toast.warning('Missing Information', 'Please fill in all required fields (Name, Ticker, and Category).');
      return;
    }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={handleBackdropClick} style={{
      ...
      <div className="absolute w-full max-w-[90%] sm:max-w-[400px] md:max-w-[500px] lg:max-w-[550px] z-0">
        <div className="relative rounded-2xl shadow-2xl">
          <img src={`${isDark? '/images/scopium/Scopium-2-dark.png' : '/images/scopium/Scopium-2.png'}`} alt="Left"
            className="absolute top-1/2 -translate-y-1/2 lg:-left-[120px] w-40 h-40 animate-pulse"
            style={{ animationDelay: '0s', animationDuration: '3s' }} />
          <img src={`${isDark? '/images/scopium/Scopium-1-dark.png' : '/images/scopium/Scopium-1.png'}`} alt="Right top"
            className="absolute lg:-top-[350px] lg:-right-[45px] translate-x-1/2 w-40 h-40 animate-pulse"
            style={{ animationDelay: '1s', animationDuration: '3s' }} />
          <img src={`${isDark? '/images/scopium/Scopium-3-dark.png' : '/images/scopium/Scopium-3.png'}`} alt="Right bottom"
      ...
      {/* Modal */}
      <div className={`relative rounded-2xl shadow-2xl w-full max-w-[90%] sm:max-w-[400px] md:max-w-[500px] lg:max-w-[550px] border z-10 transition-colors duration-300 ${
        ...
        <div className={`flex items-center justify-between p-6 border-b ${
          ...
          <h2 className={`text-xl font-semibold ${isDark ? 'text-[#F7F7F7]' : 'text-gray-800'}`}>Create Token</h2>
          <button onClick={!isCreating ? handleClose : undefined} disabled={isCreating}
            className={`p-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              isDark ? 'text-gray-300 hover:text-[#F7F7F7] hover:bg-gray-700/50' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-200'
            }`}>
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="overflow-y-auto" style={{ maxHeight: 'calc(90vh - 80px)' }}>
          <TokenForm
            refs={{ coinName, ticker, description, twitterLink, telegramLink, website, categoryBox, firstBuyAmount }}
            state={{ category, setCategory, coinImage, imageName, imageFile, bannerImage, bannerName, bannerFile, showSocialLinks, setShowSocialLinks, showBannerUpload, setShowBannerUpload, showFirstBuy, setShowFirstBuy }}
            handlers={{ handleCoinImageChange, handleBannerImageChange, removeCoinImage, removeBannerImage, handleSubmit }}
