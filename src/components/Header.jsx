...
const Header = ({ selectedNetwork, setSelectedNetwork, onSearch }) => {
  ...
  const navItems = [
    { name: 'Streamers', path: '/coming-soon', icon: '/images/icons/streamers.png' },
  ...
  return (
    <>
      <header
        ...
        style={{
          backgroundImage: isDark ? 'url(/images/header-image-dark.png)' : 'url(/images/header-image.png)',
          ...
          <div className="flex items-center md:items-end justify-between h-14 md:h-40 md:pb-4 w-full">
            ...
            <div className="flex items-center space-x-12 flex-shrink-0">
              ...
              <div className="hidden md:block relative ml-4" ref={dropdownRef}>
                ...
                >
                  <span>Create</span>
                  <ChevronDown className={`w-4 h-4 text-white transition-transform duration-200 ${isCreateDropdownOpen ? 'rotate-180' : ''}`} />
                </PinkGradientButton>

                {/* Create Dropdown Menu */}
                {isCreateDropdownOpen && (
                  <div className={`absolute top-full left-0 mt-2 w-48 rounded-lg shadow-lg border py-2 z-50 transition-colors duration-300 ${
                    isDark 
            ...
            <div className="hidden md:flex items-center space-x-6 flex-shrink-0">
              {navItems.map((item) => (
                <button
                  ...
                  className={`relative text-sm font-medium transition-colors duration-200 ${isActivePage(item.path)
                    ? `${isDark ? 'text-green-400 hover:text-green-400' : 'text-pink-300 font-bold'}`
                    : `${isDark ? 'text-[#F7F7F7] hover:text-green-400' : 'text-[#0A0A0A99] hover:text-pink-300'}`
                    }`}
                >
                  {item.name}
                  {isActivePage(item.path) && (
                    <div className={`absolute -bottom-1 left-0 right-0 h-0.5 rounded-full ${isDark? 'bg-green-400' : 'bg-pink-400'}`} />
                  )}
                </button>
              ))}
              <ThemeToggle className={`p-2 ${isDark? "text-white" : "text-[#0A0A0A99]"} hover:text-pink-300 transition-colors`}/>
