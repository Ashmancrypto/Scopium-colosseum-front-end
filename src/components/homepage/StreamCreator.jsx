...
Category: ${CATEGORIES.find(cat => cat.value === createdStream.category)?.label}
...
6. Click "Start Streaming"`
    ...
    try {
      ...
      alert('Could not copy to clipboard automatically. Please copy the details manually.')
      setDetailsCopied(true)
      onStreamCreated(createdStream)
    }
  }
      ...
      {createdStream && (
        <div className="stream-info">
          ...
          <p className="copy-instruction">
            Please copy your stream details below. The stream key is your secret, keep it safe. The modal will close after copying.
          </p>
          <div className="stream-details">
            <p><strong>Title:</strong> {createdStream.title}</p>
            {createdStream.description && (
              <p><strong>Description:</strong> {createdStream.description}</p>
            )}
            <p><strong>Category:</strong> {CATEGORIES.find(cat => cat.value === createdStream.category)?.label}</p>
            {createdStream.thumbnail_url && (
              <div className="stream-thumbnail">
                ...
                <img 
                  ...
                  style={{ maxWidth: '200px', maxHeight: '120px', objectFit: 'cover', display: 'block', marginTop: '5px' }}
                />
              </div>
            )}
            <p><strong>Stream Key:</strong> <code>{createdStream.stream_key}</code></p>
            <p><strong>RTMP URL:</strong> <code>rtmp://live.scopium.fun:1935/live</code></p>
          </div>
          ...
          <div className="obs-instructions">
            ...
            <ol>
              ...
              <li>Set Server to: <code>rtmp://live.scopium.fun:1935/live</code></li>
              <li>Set Stream Key to: <code>{createdStream.stream_key}</code></li>
